...
- Code-block wrap mode: stored in `localStorage["explorer.wrap"]` (`on` | `off`, default `on`). Read via the `useWrap` hook (`web/src/hooks/useWrap.ts`); `FileViewer` exposes a wrap toggle in its header actions for `kind === 'text'` only and passes `wrap` to `<CodeBlock>`, which switches between `.code-block.wrap-on` and `.code-block.wrap-off` styles in `FileViewer.css`. Markdown fenced code blocks are unaffected.
